...
class Follow(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    # 关注者
    follower = db.Column(db.Integer, db.ForeignKey('users.id'), index=True)
    # 被关注者
    befollowed = db.Column(db.Integer, db.ForeignKey('users.id'), index=True)
    # 被关注者头像路径
    # avatar = db.Column(db.String(200))
    # 被关注者
    # nickname = db.Column(db.String(20))
...
class Teammates(db.Model):
    ...
    # 项目id
    item_id = db.Column(db.Integer, db.ForeignKey('item_team.id'), index=True)
...
class Message(db.Model):
    ...
    # 项目发布者ID
    master_id = db.Column(db.Integer, db.ForeignKey('users.id'), index=True)
    # 申请人ID
    from_uid = db.Column(db.Integer, db.ForeignKey('users.id'), index=True)
    # 留言
...
class Chat(db.Model):
    ...
    # 接收者uid
    to_uid = db.Column(db.Integer, db.ForeignKey('users.id'), index=True)
    # 详细内容
    message = db.Column(db.String(200))
    # 发送时间
    time = db.Column(db.DateTime)
...
class BlackList(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    # 拉黑者uid
    rejecter = db.Column(db.Integer, db.ForeignKey('users.id'), index=True)
    # 被拉黑者uid
